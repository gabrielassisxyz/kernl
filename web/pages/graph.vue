<template>
  <div class="absolute inset-0 flex bg-bg-base text-text-primary overflow-hidden select-none">
    ...
    <div class="flex-1 min-w-0 relative" ref="wrapRef">
      ...
      <div class="pointer-events-none absolute inset-0 graph-aura"></div>
        ...
        <div class="ml-auto flex items-center gap-tight pointer-events-auto">
          <div class="relative">
            ...
            <input
              ...
              class="w-52 bg-surface/70 backdrop-blur border border-border-hairline rounded-lg pl-8 pr-3 py-1.5 text-body text-text-primary placeholder-text-faint focus:outline-none focus:border-da-accent transition-colors"
            />
          </div>
          <button @click="fitView" title="Fit to view" class="ctrl-btn"><span class="material-symbols-outlined text-[18px]">fit_screen</span></button>
          <button @click="reload" title="Reload graph" class="ctrl-btn"><span class="material-symbols-outlined text-[18px]" :class="loading && 'animate-spin'">{{ loading ? 'progress_activity' : 'refresh' }}</span></button>
        </div>
      </header>
      ...
      <div class="absolute bottom-0 left-0 z-20 px-margin py-component flex flex-wrap items-center gap-x-component gap-y-tight max-w-[70%]">
        <button
          ...
          class="group flex items-center gap-tight font-mono-data text-mono-data transition-opacity"
          :class="hiddenTypes.has(t.type) ? 'opacity-35' : 'opacity-100'"
        >
          <span class="w-2.5 h-2.5 rounded-full transition-transform group-hover:scale-125" :style="{ background: t.color, boxShadow: `0 0 8px ${t.color}99` }"></span>
      ...
      >
        <g :transform="`translate(${panX},${panY}) scale(${scale})`">
          ...
          <g>
            ...
            >
              <!-- soft glowing aura, brighter when active/selected -->
              <circle :r="n.r * 2.6" :fill="colorFor(n.type)" :opacity="(selectedId === n.id || n.id === hoverId) ? 0.32 : 0.16" :style="{ transition: 'opacity 150ms' }" />
              <!-- companion-note affordance: a thin dashed orbit ring marks a node
                   that absorbed an annotation note, brightening when active -->
              <circle
                v-if="n.hasNote"
              ...
              />
              <!-- solid colored core carries the type identity -->
              <circle
                :r="n.r"
              ...
              />
              <!-- small offset specular highlight for a lit, dimensional feel -->
      ...
      >
        <div class="-translate-x-1/2 -translate-y-[calc(100%+14px)] bg-surface/95 backdrop-blur border border-border-default rounded-lg px-3 py-2 shadow-[0_12px_40px_-12px_rgba(0,0,0,0.8)]">
          <div class="flex items-center gap-tight mb-0.5">
            ...
            <span class="font-label-caps text-label-caps uppercase text-text-muted">{{ label(hoverNode.type) }}</span>
          </div>
      ...
      <div v-if="loading" class="absolute inset-0 flex items-center justify-center font-mono-data text-mono-data text-text-muted">
        <span class="material-symbols-outlined animate-spin mr-2">progress_activity</span> mapping the graph…
      </div>
      <div v-else-if="error" class="absolute inset-0 flex items-center justify-center font-mono-data text-mono-data text-status-failed">
        {{ error }}
      </div>
    ...
    <Transition name="panel">
      <aside
        v-if="selectedNode"
        class="w-[340px] shrink-0 h-full border-l border-border-hairline bg-surface/80 backdrop-blur-xl flex flex-col z-30"
      >
        <div class="flex items-start gap-component px-component py-component border-b border-border-hairline">
          <span class="mt-1 w-3 h-3 rounded-full shrink-0" :style="{ background: colorFor(selectedNode.type), boxShadow: `0 0 10px ${colorFor(selectedNode.type)}` }"></span>
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-tight">
              <span class="font-label-caps text-label-caps uppercase text-text-muted">{{ label(selectedNode.type) }}</span>
              <span
                v-if="selectedNode.hasNote"
                title="has an associated note"
                class="inline-flex items-center gap-0.5 font-label-caps text-label-caps uppercase text-text-faint border border-border-hairline rounded-full px-1.5 leading-[1.4]"
              >
          ...
          <button @click="selectedId = ''" class="ctrl-btn shrink-0"><span class="material-symbols-outlined text-[18px]">close</span></button>
        </div>
...
const panY = ref(0)
const scale = ref(1)
const frame = ref(0) // bumped each sim tick to drive SVG re-render off plain arrays
...
.graph-aura {
  background:
    radial-gradient(1100px 700px at 28% 18%, rgba(123, 143, 224, 0.10), transparent 60%),
    radial-gradient(900px 600px at 82% 88%, rgba(95, 168, 196, 0.07), transparent 55%);
}
.graph-grid {
  background-image:
...
.stat { display: flex; flex-direction: column; gap: 2px; padding: 8px 12px; border: 1px solid var(--color-border-hairline); border-radius: 8px; background: var(--color-surface-container-low); }
.stat-num { font-family: var(--font-mono-data); font-size: 20px; line-height: 1; color: var(--color-text-primary); }
.stat-lbl { font-family: var(--font-mono-data); font-size: 11px; text-transform: uppercase; letter-spacing: 0.08em; color: var(--color-text-faint); }

.conn-head { font-family: var(--font-label-caps); font-size: var(--text-label-caps); letter-spacing: 0.12em; text-transform: uppercase; color: var(--color-text-faint); margin-bottom: 8px; }
.conn-row {
...
.conn-label { font-family: var(--font-mono-data); font-size: 11px; color: var(--color-da-accent); white-space: nowrap; }
