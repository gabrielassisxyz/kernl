<template>
  <div class="group flex flex-col px-section py-component border-b border-border-hairline hover:bg-surface-hover transition-colors duration-150 cursor-pointer relative"
    ...
    <div class="flex flex-col gap-tight">
      <p class="font-body text-text-muted truncate text-[13px]">{{ item.Description || item.Excerpt || item.URL }}</p>
      <div class="flex gap-tight mt-1 items-center">
        <span class="font-mono-data text-[10px] text-text-faint truncate max-w-[200px]">{{ domain(item.URL) }}</span>
        <div v-if="item.Tags && item.Tags.length > 0" class="flex gap-tight">
          <span class="font-mono-data text-[10px] text-text-dim bg-surface border border-border-hairline px-tight py-[1px]" v-for="tag in item.Tags.slice(0, 2)" :key="tag">
            {{ tag }}
          </span>
          <span class="font-mono-data text-[10px] text-text-faint" v-if="item.Tags.length > 2">
            +{{ item.Tags.length - 2 }}
          </span>
        </div>
      </div>
