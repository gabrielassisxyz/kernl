...
content-search: pattern="module:.*(my-module|skill-design)" path=docs/solutions/ files_only=true case_insensitive=true
...
When no relevant learnings are found, say so explicitly, include the search context so the caller can see what was looked for, and note that the caller's work may be worth capturing with `/save` after it lands — the absence is itself useful signal.
...
- `/vc-plan` — to inform planning with institutional knowledge and add depth during confidence checking
- `/vc-ideate` — to surface prior learnings relevant to the ideation topic
