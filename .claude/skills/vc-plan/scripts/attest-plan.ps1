...
function Resolve-PlanFile {
    ...
    if ($env:PLAN_ID) {
        ...
        $planFile  = Join-Path $candidate "docs\task_plan.md"
    ...
    if (Test-Path -LiteralPath $planRoot) {
        $newest = Get-ChildItem -LiteralPath $planRoot -Directory -ErrorAction SilentlyContinue |
            ...
            Where-Object { (Test-Path -LiteralPath (Join-Path $_.FullName "docs\task_plan.md")) -or (Test-Path -LiteralPath (Join-Path $_.FullName "task_plan.md")) } |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1
        if ($newest) {
            $planFile = Join-Path $newest.FullName "docs\task_plan.md"
            if (Test-Path -LiteralPath $planFile) { return (Resolve-Path -LiteralPath $planFile).Path }
            $planFile = Join-Path $newest.FullName "task_plan.md"
            return (Resolve-Path -LiteralPath $planFile).Path
        }
    }

    $docsPath = Join-Path (Get-Location) "docs\task_plan.md"
    if (Test-Path -LiteralPath $docsPath) {
        return (Resolve-Path -LiteralPath $docsPath).Path
    }
...
function Get-AttestationPath {
    ...
    # If plan is in docs/, store attestation in docs/ alongside it
