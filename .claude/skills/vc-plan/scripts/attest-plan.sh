#!/bin/sh
# vc-plan: lock the current task_plan.md content with a SHA-256 attestation.
#
...
resolve_plan_file() {
    ...
    if [ -n "${plan_dir}" ]; then
        if [ -f "${plan_dir}/docs/task_plan.md" ]; then
            printf "%s\n" "${plan_dir}/docs/task_plan.md"
            return 0
...
attestation_path_for() {
    ...
    if [ "${plan_dir}" = "." ]; then
        ...
        printf "%s\n" "./.plan-attestation"
    elif [ "$(basename "${plan_dir}")" = "docs" ]; then
        # docs/ mode: store attestation in docs/ alongside plan
