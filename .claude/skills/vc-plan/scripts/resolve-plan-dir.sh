#!/bin/sh
# vc-plan: resolve active plan directory.
#
# Resolution order:
...
resolve_latest_dir() {
    ...
    for entry in "${PLAN_ROOT}"/*/; do
        ...
        # Skip dirs that are not plan dirs (check docs/ first, then legacy root)
        [ -f "${clean}/docs/task_plan.md" ] || [ -f "${clean}/task_plan.md" ] || continue
