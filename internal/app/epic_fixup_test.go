...
// DriveEpicIntegrationTail exercises: Get/Update/Rewind to drive the epic
// bead itself, and a real, persisting Create - unlike every other fake
// BackendPort in this package, whose Create is a no-op, because this is the
// one test that actually needs to observe what gets created.
...
func (b *fixupFakeBackend) List(_ *backend.BeadListFilters, _ string) ([]backend.Bead, error) {
	return nil, nil
}
...
func epicIntegrationTailDeps(t *testing.T, be *fixupFakeBackend, epicID string) (DriveEpicIntegrationTailDeps, string) {
	t.Helper()
	stateDir := t.TempDir()
	be.put(&backend.Bead{ID: epicID, Title: "the epic", State: "integration_review", ProfileID: "epic"})
...
func TestDriveEpicIntegrationTail_FixupCreatesLinkedBeadAndRewinds(t *testing.T) {
	...
	if !epicAlreadyHasFixup(epicBead.Description) {
		t.Error("the epic's description must carry the fix-up-created marker, so a second rejection escalates")
	}
...
func TestDriveEpicIntegrationTail_SecondRejectionEscalatesEvenWhenDeclaredFixup(t *testing.T) {
	...
	// A later run: the epic is back at integration_review (a second attempt
	// after the fix-up bead's own branch got merged) and integration_review
	// rejects again - and this time it ALSO declares "fixup", the same
	// clean declaration as the first time.
