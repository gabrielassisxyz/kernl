...
- **Review and iterate in chat** — present the saved ideation file, collect feedback, apply changes, and loop back.
- **Brainstorm a selected idea** — go deeper on one idea through dialogue. Unlike repo mode, this is not the first step of an implementation chain — there is no `vc-plan` after; `vc-brainstorm` in universal mode develops the idea further (e.g., expands a name into a brand brief, a plot into an outline, a decision into a weighed framework) and ends there. Persist first per the §6.3 contract in `references/post-ideation-workflow.md`: save the survivors to `docs/` (the elsewhere-mode default), mark the chosen idea as `Explored`, then load `vc-brainstorm` with that idea as the seed. If the file save fails, apply the §6.5 Failure Recovery (in `references/post-ideation-workflow.md`) before ending so the brainstorm seed is preserved.
- **Save and end** — save the survivors to a local file in `docs/`, then end.
