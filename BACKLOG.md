...
3. **Version the vault**, not a plain private git repo: Gabriel wants `~/vault`'s personal notes protected even from a "private" GitHub/GitLab repo being flipped public by mistake. Decided mechanism (2026-07-24): local git + `git-remote-gcrypt` so any remote provider only ever receives ciphertext, plus periodic backups to `/tank` and Hetzner, plus a machine-wide hook blocking every way an agent could change this repo's visibility. Tracked as its own item in `llm-workflow/BACKLOG.md` ("Version `~/vault` with an encrypted remote") since it is real infra work, not a one-liner; still the unblock for steps 4 to 5 once it lands (history/blame is what makes moving about-me and ops logs acceptable, see the 2026-07-11 incident that founded the machine-log rule).
