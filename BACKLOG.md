...
**Update 2026-07-24, this "must not require serve" requirement did not survive contact with how CLI parity actually shipped.** The parity track below built the new verbs as thin REST clients, not direct-to-DB, so there was no direct-to-DB path left to depend on instead. The shipped `kernl-da` starts a server idempotently when none answers; see the DA absorption track's step 2 for the full account.
...
1. **Agent-ergonomics pass on the CLI**, see *A complete, agent-oriented CLI*. Done.
2. **`kernl da` v1**, see the decided mechanism under *When the DA's harness is Claude…*: exact-same-experience wrapper, cwd anchored at llm-workflow, launchable anywhere, preamble deferring to `kernl --help`. **Done 2026-07-24**, as `llm-workflow/bin/kernl-da` (lives there, not here, on purpose, see that script's own header on why it is glue kernl's absorption is meant to replace, not migrate). Ships two decisions that revise this paragraph: it **does** require `kernl serve` running (starts one idempotently if none answers, since the CLI-parity track below shipped the new verbs as thin REST clients, so the direct-to-DB alternative this paragraph assumed never existed), and it runs both harnesses with sandbox/permission checks off (`claude --dangerously-skip-permissions`, `codex --yolo`), the only fix found for Codex's default sandbox denying writes to `~/vault` (neither the working root nor an `--add-dir`) regardless of `KERNL_CONFIG` (the env var kernl gained for exactly this: a caller with cwd anchored elsewhere).
3. **Version the vault**, not a plain private git repo: Gabriel wants `~/vault`'s personal notes protected even from a "private" GitHub/GitLab repo being flipped public by mistake. **Decided 2026-07-25:** local git + `git-remote-gcrypt` pushing to the Hetzner Storage Box over `rsync://`, encrypted with a GPG key dedicated to this alone. The periodic `/tank` and Hetzner backups originally listed here turned out to already exist and already cover `~/vault` (the nightly root restic timer), and the anti-publication hook was dropped because a Storage Box has no visibility setting for a hook to guard. Full plan, findings and open items in `llm-workflow/BACKLOG.md` ("Version `~/vault` with an encrypted remote"); still the unblock for steps 4 to 5 once it lands (history/blame is what makes moving about-me and ops logs acceptable, see the 2026-07-11 incident that founded the machine-log rule).
