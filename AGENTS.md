...
- **Orchestrator storage:** per repository, declared by `registry.repos[].memoryManager`. Two trackers are supported: **`br`** (beads_rust ≥ 0.2.10, SQLite) and **`bd`** (gastownhall/beads ≥ 1.0.4, Dolt embedded). `br` is what the repositories this orchestrator serves actually use, so it is the one a new target repository will need. For a target repository this is the *product's* runtime store for executing epics. For **this** repository it is also the dev-task tracker (§6), and that is deliberate: one repo has one `.beads/`, and pointing `registry.repos[]` at it is what lets `kernl bead run` execute this project's own work. What must not be borrowed is the orchestrator's *schema* - epic stages, gates and attempt ledgers describe a run, not a backlog. `bv` is not used and stays out. `.beads/` is gitignored (no task data in the public repo).
...
- **Task tracking: `br`, and nothing else.** This project's work lives in the beads tracker (`.beads/beads.db`, issue prefix `kernl`, gitignored). Start any session with `br ready`, and keep it true in the same session: starting, finishing, blocking or deciding a unit of work all go in, or the next session repeats it. A bead's anatomy, the quality gate and the dependency rules are the `writing-beads` norm; `br lint` and that gate are what decide whether a bead is claimable. A blocker is an edge or a status, never a sentence in the description - `br ready` reads the graph, not prose.
- **One surface, and this is the second time it has been decided.** The markdown backlog in `local/` was retired on 2026-07-30 because it and the graph were live at once, and 8 of its 23 open entries existed in both, under different names in different languages. The `kernl` project in the graph won then and lost on 2026-08-21, for a reason the markdown file never had: it carries no dependency edges, so it cannot tell an unattended agent what to run first, and the orchestrator reads `br`. The 120 tasks open at that point were triaged against the code - 9 were already implemented and closed, the rest convert. **Tasks already in the graph stay readable while they convert; nothing new is written there.** An item saved to the wrong surface is an item lost.
- **There is deliberately no published roadmap.** A statement of direction nobody maintains reads as a promise, and at pre-1.0 with one user there is no direction stable enough to publish. If one earns its place later it comes back; until then, what the project does is what the README and the code say it does.
